...
def read_modules(
    ...
    query = db.query(models.Module).options(
        joinedload(models.Module.specializations)
    )
    #  ADMIN → see all
    if r == "admin":
        rows = query.all()
    #  PM → only modules of managed programs
    elif r == "pm":
        program_ids = hosp_program_ids(db, current_user)
        rows = query.filter(models.Module.program_id.in_(program_ids)).all()

    #  LECTURER → only assigned modules
    elif r == "lecturer":
        rows = (
            query
            .join(models.LecturerModule)  # your lecturer-module link table
            .filter(models.LecturerModule.lecturer_id == current_user.lecturer_id)
            .all()
        )

    #  STUDENT → only modules from their program
    elif r == "student":
        rows = query.all()
...
def create_module(
    ...
    if r == "admin":
        ...
        if not current.user.lecturer_id:
            raise HTTPException(status_code=403, detail="Not allowed")
        program = db.query(models.StudyProgram).filter(
            models.StudyProgram.id == p.program_id
        ).first()
        if not program or program.head_of_program_id != current_user.lecturer_id:
            raise HTTPException(
                status_code=403,
                detail="You can only create modules for your program"
            )
...
def update_module(
    ...
    if r == "admin":
        ...
        if not current_user.lecturer_id:
            raise HTTPException(status_code=403, detail="Not allowed")

        program = db.query(models.StudyProgram).filter(
            models.StudyProgram.id == row.program_id
        ).first()

        if not program or program.head_of_program_id != current_user.lecturer_id:
            raise HTTPException(
                status_code=403,
                detail="You can only update modules of your program"
            )
    elif r in ["lecturer", "student"]:
        raise HTTPException(status_code=403, detail="Not allowed")
...
def delete_module(
    ...
    if r == "admin":
        ...
        if not program or program.head_of_program_id != current_user.lecturer_id:
            raise HTTPException(
                status_code=403,
                detail="You can only delete modules of your program"
            )
    elif r in ["lecturer", "student"]:
        raise HTTPException(status_code=403, detail="Not allowed")
